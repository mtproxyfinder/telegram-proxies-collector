...
def get_iran_nodes() -> List[str]:
    """Retrieve Iranian nodes from Check Host API."""
    response = requests.get("https://check-host.net/nodes/hosts", headers={"Accept": "application/json"})
    if response.status_code == 200:
        nodes = response.json().get("nodes", {})
        iran_nodes = [name for name, details in nodes.items() if "ir" in details["location"]]
        return iran_nodes
    else:
        print(f"Failed to retrieve nodes: {response.status_code} - {response.text}")
        return []

def check_ip_with_nodes(ip: str, nodes: List[str]) -> bool:
    """Check the validity of an IP address using the Check Host API."""
    node_params = "&node=".join(nodes)
    url = f"https://check-host.net/check-http?host=http://{ip}:443&node={node_params}"
    ...
    if response.status_code == 200:
        ...
        if "request_id" in result:
            ...
            print(f"Request ID: {request_id}. Waiting for results...")
            time.sleep(10)  # Wait for a while before checking results

            # Check the result of the request
            result_check = check_result(request_id)
            for node, checks in result_check.items():
                try:
                    for check in checks:
                        status = check[2]  # The first element indicates success (1) or failure (0)
                        if status != 'Connection refused':
                            print(ip + " is not valid! (check-host)")
                            return False  # IP is not valid
                except (TypeError, IndexError, ValueError) as e:
                    print(f"Error processing checks for {ip}: {e}")
                    return False
    print(ip + " is valid! (check-host)")
    return True  # IP is Valid!

def check_result(request_id: str) -> dict:
    """Check the result of the HTTP check request."""
...
for proxy in array_proxies:
    try:
        ...
        if not check_ip_with_nodes(server, iran_nodes):
            continue
